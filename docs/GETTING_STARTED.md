...
If you prefer letting an AI agent do this for you, just ask: **"setup harness-on"**. The agent will read `node_modules/oh-my-harness-loop/docs/SETUP_INSTRUCTIONS_FOR_AGENT.md` and walk through the steps below. Skip to "Customize for your project" once it's done.
...
If the shims are missing, the postinstall failed silently. Re-run `npm install oh-my-harness-loop@latest`, or create the files manually (see [README §Slash Commands](../README.md#slash-commands-auto-installed)).
...
```jsonc
{
  "plugin": ["oh-my-harness-loop@latest"]
}
```
...
The package ships starter templates at `node_modules/oh-my-harness-loop/templates/init/`. Copy them into your project:
...
# Runner stub
mkdir -p scripts
cp node_modules/oh-my-harness-loop/templates/init/scripts/harness-check.sh scripts/
chmod +x scripts/harness-check.sh

# Gate docs (one per gate)
mkdir -p docs/harness/gates
cp node_modules/oh-my-harness-loop/templates/init/docs/harness/gates/*.md docs/harness/gates/
