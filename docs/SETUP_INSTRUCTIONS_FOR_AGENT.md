...
1. Confirm `oh-my-harness-loop` is in the project's plugin list:
   - Read `.opencode/opencode.json` if it exists.
   - If `plugin` array does not contain `oh-my-harness-loop@latest`, ask the user before proceeding.
2. Confirm the package is installed at the expected location (`node_modules/oh-my-harness-loop/` exists). If not, run `npm install oh-my-harness-loop@latest`.
...
The template lives at `node_modules/oh-my-harness-loop/templates/init/`. Copy from there into the project root.

### 1. `.opencode/harness.config.json`
Source: `node_modules/oh-my-harness-loop/templates/init/.opencode/harness.config.json`
...
Source: `node_modules/oh-my-harness-loop/templates/init/scripts/harness-check.sh`
...
Source: `node_modules/oh-my-harness-loop/templates/init/docs/harness/gates/`
...
Source: `node_modules/oh-my-harness-loop/templates/init/gitignore.template`
...
3. **Optional**: enable epic mode in `harness.config.json` if running multi-story workflows. See `node_modules/oh-my-harness-loop/README.md#epic-mode`.
...
Full user-facing walkthrough: `node_modules/oh-my-harness-loop/docs/GETTING_STARTED.md`
